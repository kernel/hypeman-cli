...
var composeUpCmd = cli.Command{
	...
	Action: func(ctx context.Context, cmd *cli.Command) error {
		...
		result, err := runner.up(ctx, composeUpOptions{
...
func newComposeRunner(cmd *cli.Command) (*composeRunner, error) {
...
func (r *composeRunner) plan(ctx context.Context) (composePlan, error) {
	desiredInstances, desiredIngresses, images, err := r.desiredResources()
	if err != nil {
		return composePlan{}, err
	}

	var actions []composeAction
...
func (r *composeRunner) up(ctx context.Context, opts composeUpOptions) (composePlan, error) {
...
func (r *composeRunner) down(ctx context.Context, verbose bool) (composePlan, error) {
	...
	for _, ing := range ingresses {
		actions = append(actions, composeAction{
			Action:    "delete",
			Type:      "ingress",
			Name:      ing.Name,
	...
	if len(actions) == 0 {
		...
		for _, ingress := range desiredIngresses {
			result.Actions = append(result.Actions, composeAction{
				Action:  "skip",
				Type:    "ingress",
				Name:    ingress.Name,
...
func (r *composeRunner) applyCreate(ctx context.Context, action *composeAction, opts composeUpOptions) error {
	switch action.Type {
	case "image":
...
func (r *composeRunner) applyReplace(ctx context.Context, action *composeAction, opts composeUpOptions) error {
	switch action.Type {
	case "instance":
...
func (r *composeRunner) ensureImageReady(ctx context.Context, image string, verbose bool) error {
...
func (r *composeRunner) waitForInstanceRunning(ctx context.Context, instanceID, timeout string, verbose bool) error {
	if verbose {
...
func (r *composeRunner) desiredResources() ([]desiredInstance, []desiredIngress, []string, error) {
...
func (r *composeRunner) planImage(ctx context.Context, image string) (composeAction, error) {
	_, err := r.client.Images.Get(ctx, url.PathEscape(image), r.opts...)
	if err == nil {
		return composeAction{Action: "unchanged", Type: "image", Name: image, Reason: "already exists"}, nil
	}
	if isHTTPNotFound(err) {
		return composeAction{Action: "create", Type: "image", Name: image, Reason: "not present"}, nil
	}
	return composeAction{}, fmt.Errorf("check image %s: %w", image, err)
}

func planInstanceAction(desired desiredInstance, owned []hypeman.Instance, all []hypeman.Instance) composeAction {
...
func planIngressAction(desired desiredIngress, owned []hypeman.Ingress, all []hypeman.Ingress) composeAction {
	action := composeAction{
		Type:         "ingress",
		Name:         desired.Name,
		Service:      desired.Service,
...
func (r *composeRunner) listComposeInstances(ctx context.Context) ([]hypeman.Instance, error) {
...
func (r *composeRunner) listComposeIngresses(ctx context.Context) ([]hypeman.Ingress, error) {
...
func replacementBlockers(actions []composeAction, replace bool) []string {
	if replace {
		return nil
	}
	var blockers []string
...
func conflictBlockers(actions []composeAction) []string {
	var blockers []string
	for _, action := range actions {
...
func summarizeComposeActions(actions []composeAction) composeSummary {
...
func showComposeResult(cmd *cli.Command, title string, result composePlan) error {
...
func printComposePlan(result composePlan) {
...
func printComposeDone(result composePlan) {
	fmt.Fprintln(os.Stderr)
	printComposeSummaryTo(os.Stderr, "Done", result.Summary)
}

func printComposeSummary(summary composeSummary) {
	printComposeSummaryTo(os.Stdout, "Summary", summary)
}

func printComposeSummaryTo(out *os.File, label string, summary composeSummary) {
	parts := []string{}
	if summary.Create > 0 {
		parts = append(parts, fmt.Sprintf("%d to create", summary.Create))
	}
	if summary.Replace > 0 {
		parts = append(parts, fmt.Sprintf("%d to replace", summary.Replace))
	}
	if summary.Delete > 0 {
		parts = append(parts, fmt.Sprintf("%d to delete", summary.Delete))
	}
	if summary.Unchanged > 0 {
		parts = append(parts, fmt.Sprintf("%d unchanged", summary.Unchanged))
	}
	if summary.Skip > 0 {
		parts = append(parts, fmt.Sprintf("%d skipped", summary.Skip))
	}
	if summary.Conflict > 0 {
		parts = append(parts, fmt.Sprintf("%d conflicts", summary.Conflict))
	}
	if len(parts) == 0 {
...
func isHTTPNotFound(err error) bool {
	...
	return isNotFoundError(err, &apiErr)
}

func sortComposeActions(actions []composeAction) {
